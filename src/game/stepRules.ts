...
export function getStepRatio(steps: number, goal = DEFAULT_STEP_GOAL) {
  if (!goal) return 0;
  return Math.max(0, steps / goal);
}

export function getStepProgress(steps: number, goal = DEFAULT_STEP_GOAL) {
...
export function getCharacterStatus(steps: number, goal = DEFAULT_STEP_GOAL) {
