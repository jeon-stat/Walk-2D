...
export const DEFAULT_FRIEND_GROUPS: FriendGroup[] = [
...
export function buildFriendRankingData({
  ...
  currentUser?: { handle?: string | null; nickname?: string | null } | null;
  todayRecord?: { steps?: number } | null;
  weeklySteps?: number;
  streak?: number;
  energyLevel?: number;
  longTermState?: string;
  ...
  const friends: FriendCard[] = [...BASE_FRIENDS];
...
export function createFriendGroupState(friends: Array<{ id: string; groupIds?: ReadonlyArray<string> | string[] }> = []) {
...
export function getFriendGroupIds(
  friend: { id?: string; groupIds?: ReadonlyArray<string> | string[] },
  groupState: Record<string, string[]> = {},
) {
  return normalizeGroupIds(groupState?.[friend.id ?? ""] ?? friend?.groupIds ?? ["all"]);
}

export function getFriendGroupNames(
  friend: { id?: string; groupIds?: ReadonlyArray<string> | string[] },
  groupState: Record<string, string[]> = {},
  groups: ReadonlyArray<FriendGroup> = DEFAULT_FRIEND_GROUPS,
) {
...
export function filterFriendsByGroup<T extends { groupIds?: ReadonlyArray<string> | string[] }>(friends: T[], groupId: string) {
  if (groupId === "all") {
    return [...friends];
  }
...
export function toggleFriendGroupMembership(
  groupState: Record<string, string[]>,
  friendId: string,
  groupId: string,
) {
...
export function sortFriendCards<T extends { nickname?: string; [key: string]: unknown }>(friends: T[], mode: string) {
  const sorted = [...friends];
  const scoreKey = getSortKey(mode);

  return sorted.sort((a, b) => {
    const diff = Number((b?.[scoreKey] as number) ?? 0) - Number((a?.[scoreKey] as number) ?? 0);
    if (diff !== 0) {
      return diff;
    }
...
export function getFriendSortLabel(mode: string) {
  switch (mode) {
    case "weekly":
      return "주간 누적 발걸음 순위";
    case "streak":
...
function normalizeGroupIds(groupIds: ReadonlyArray<string> | string[] | undefined | null) {
  const normalized = new Set(Array.isArray(groupIds) ? groupIds.filter(Boolean) : []);
  normalized.add("all");
  return Array.from(normalized);
}

function getSortKey(mode: string) {
  switch (mode) {
    case "weekly":
      return "weeklySteps";
    case "streak":
